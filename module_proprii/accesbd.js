...
class AccesBD {
    ...
    select({ tabel = "", campuri = [], conditiiAnd = [] } = {}, callback, parametriQuery = []) {
        // select({tabel:"prajituri", campuri:["nume", "pret", "descriere"], conditiiAnd:["pret>10", "calorii<700"]}, function(err, rez){})
        let conditieWhere = "";
        if (conditiiAnd.length > 0)

            conditieWhere = `where ${conditiiAnd.join(" and ")}`; //where pret>10 and calorii<700
